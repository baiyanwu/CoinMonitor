...
// NAMESPACE OBJECT: ./node_modules/lightweight-charts/dist/lightweight-charts.development.mjs
var lightweight_charts_development_namespaceObject = {};
__webpack_require__.r(lightweight_charts_development_namespaceObject);
__webpack_require__.d(lightweight_charts_development_namespaceObject, {
...
class PriceAxisWidget {
    constructor(pane, options, rendererOptionsProvider, side) {
        ...
        const handler = {
            ...
            _internal_mouseLeaveEvent: this._private__mouseLeaveEvent.bind(this),
            // Do not let two-finger gestures on the price axis fall through to chart pinch scaling.
            _internal_pinchStartEvent: this._private__mouseUpEvent.bind(this),
            _internal_pinchEvent: () => { },
            _internal_pinchEndEvent: () => { },
        };
...
function lightweight_charts_development_size(barSpacing, coeff) {
...
function shapeSize(shape, originalSize) {
    switch (shape) {
        case 'arrowDown':
        case 'arrowUp':
            return lightweight_charts_development_size(originalSize, 1);
        case 'circle':
            return lightweight_charts_development_size(originalSize, 0.8);
        case 'square':
            return lightweight_charts_development_size(originalSize, 0.7);
    }
}
function calculateShapeHeight(barSpacing) {
    return ceiledEven(lightweight_charts_development_size(barSpacing, 1));
}
function shapeMargin(barSpacing) {
    return Math.max(lightweight_charts_development_size(barSpacing, 0.1), 3 /* Constants.MinShapeMargin */);
}
...
;// ./lib/app/pane/pane-cache.js
class PaneCache extends Map {
...
class GetPane extends SeriesInstanceMethod {
    constructor(paneCache) {
        super("getPane", function (series, params, resolve) {
            const pane = series.getPane();
            try {
                resolve(paneCache.getKeyOfPane(pane));
            } catch (e) {
                const paneId = `${params.seriesId}-pane`;
                paneCache.set(paneId, pane);
                resolve(paneId);
            }
        });
    }
...
;// ./lib/app/pane/pane-instance.js



class PaneInstanceService {
...
class LocatorComponent {
    ...
    init(functionManager, pluginManager, chart) {
        ...
        this.registerPaneInstanceService();
    ...
    registerPaneInstanceService() {
        Locator.register(PaneInstanceService.name, () => new PaneInstanceService(Locator));
    }
...
class ChartRegistrationFunctionsController {
    ...
    registerFunctions() {
        ...
        priceScale.register()
        const paneInstance = Locator.resolve(PaneInstanceService.name)
        paneInstance.register()
...
const app_createChart = createChart
window.LightweightCharts = Object.assign({}, lightweight_charts_development_namespaceObject)
